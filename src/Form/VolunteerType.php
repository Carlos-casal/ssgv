    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $builder
            ...
            ->add('city', ChoiceType::class, [
                'label' => 'Población', 'required' => true,
                'placeholder' => 'Población', 'choices' => [], // Loaded by JS
            ])
            ...
            ->add('drivingLicenses', ChoiceType::class, [
                ...
                'choices' => [ 'A1' => 'A1', 'A' => 'A', 'B' => 'B', 'C1' => 'C1', 'C' => 'C', 'D1' => 'D1', 'D' => 'D', 'EC' => 'EC' ],
                'multiple' => true, 'expanded' => true, 'required' => false,
            ])
            ->add('drivingLicenseExpiryDate', DateType::class, [
                'label' => 'Fecha de Caducidad (Permiso Conducción)', 'widget' => 'single_text', 'html5' => true, 'required' => false,
            ])
            ->add('navigationLicenses', ChoiceType::class, [
                'label' => 'Permisos de Navegación',
                'choices' => [ 'Licencia de Navegación (LN)' => 'LN', 'PNB' => 'PNB', 'PER' => 'PER', 'Patrón de Yate (PY)' => 'PY', 'Capitán de Yate (CY)' => 'CY' ],
                'multiple' => true, 'expanded' => true, 'required' => false,
            ])
            ->add('specificQualifications', ChoiceType::class, [
                'label' => 'Titulaciones Específicas',
                'choices' => [ 'TES (Técnico en Emergencias Sanitarias)' => 'TES', 'TTS (Técnico en Transporte Sanitario)' => 'TTS', 'Enfermería' => 'Enfermeria', 'DUE (Diplomado Universitario en Enfermería)' => 'DUE', 'Médico' => 'Medico' ],
                'multiple' => true, 'expanded' => true, 'required' => false,
            ])
        ...
        $builder->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) {
            ...
            // Make 'previousVolunteeringInstitutions' required if 'hasVolunteeredBefore' is true
            if (isset($data['hasVolunteeredBefore']) && $data['hasVolunteeredBefore'] === '1') {
                $form->add('previousVolunteeringInstitutions', TextareaType::class, [
                    ...
                    'attr' => ['rows' => 4, 'placeholder' => 'Ej: Cruz Roja, Cáritas...'],
                ]);
            }

            // Make 'drivingLicenseExpiryDate' required if any driving license is selected
            if (!empty($data['drivingLicenses'])) {
                $form->add('drivingLicenseExpiryDate', DateType::class, [
                    'label' => 'Fecha de Caducidad (Permiso Conducción)',
                    'widget' => 'single_text',
                    'html5' => true,
                    'required' => true,
                    'constraints' => [new NotBlank(['message' => 'Debe indicar la fecha de caducidad si selecciona un permiso.'])],
                ]);
            }
        });
    }
